...
import { validationRequests, ProficiencyLevel } from "../data/mockData";
import React from "react";

export function SkillValidation() {
  const [selectedRequest, setSelectedRequest] = useState(validationRequests[0]);
  const [feedback, setFeedback] = useState("");
  const [adjustedLevel, setAdjustedLevel] = useState<ProficiencyLevel>(selectedRequest.requestedProficiency);
              ...
              <div className="mb-4">
                ...
                {adjustedLevel !== selectedRequest.requestedProficiency && (
                  <p className="text-sm text-amber-600 mt-2">
                    ⚠️ You are adjusting the proficiency level from {selectedRequest.requestedProficiency} to {adjustedLevel}
                  </p>
                )}
              </div>
